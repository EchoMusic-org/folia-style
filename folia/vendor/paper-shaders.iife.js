(()=>{var zo=Object.defineProperty;var Eo=(o,e)=>{for(var a in e)zo(o,a,{get:e[a],enumerable:!0})};var Ve={};Eo(Ve,{DitheringShapes:()=>eo,DitheringTypes:()=>oo,DotGridShapes:()=>Qe,GemSmokeShapes:()=>Io,GlassDistortionShapes:()=>po,GlassGridShapes:()=>mo,GrainGradientShapes:()=>ao,HalftoneCmykTypes:()=>Uo,HalftoneDotsGrids:()=>So,HalftoneDotsTypes:()=>wo,LiquidMetalShapes:()=>bo,PulsingBorderAspectRatios:()=>ro,ShaderFitOptions:()=>ze,ShaderMount:()=>fe,WarpPatterns:()=>je,colorPanelsFragmentShader:()=>so,colorPanelsMeta:()=>ne,defaultObjectSizing:()=>Me,defaultPatternSizing:()=>Oe,ditheringFragmentShader:()=>$e,dotGridFragmentShader:()=>Ne,dotOrbitFragmentShader:()=>Te,dotOrbitMeta:()=>pe,emptyPixel:()=>Mo,flutedGlassFragmentShader:()=>fo,gemSmokeFragmentShader:()=>Vo,gemSmokeMeta:()=>Ce,getShaderColorFromString:()=>Fo,getShaderNoiseTexture:()=>Ro,godRaysFragmentShader:()=>Je,godRaysMeta:()=>he,grainGradientFragmentShader:()=>to,grainGradientMeta:()=>_e,halftoneCmykFragmentShader:()=>Bo,halftoneDotsFragmentShader:()=>Co,heatmapFragmentShader:()=>xo,heatmapMeta:()=>be,imageDitheringFragmentShader:()=>go,isPaperShaderElement:()=>Re,lensDistortionFragmentShader:()=>vo,lensDistortionMeta:()=>Be,liquidMetalFragmentShader:()=>yo,meshGradientFragmentShader:()=>De,meshGradientMeta:()=>me,metaballsFragmentShader:()=>Ye,metaballsMeta:()=>re,neuroNoiseFragmentShader:()=>Ge,paperTextureFragmentShader:()=>co,perlinNoiseFragmentShader:()=>Le,pulsingBorderFragmentShader:()=>io,pulsingBorderMeta:()=>se,simplexNoiseFragmentShader:()=>We,simplexNoiseMeta:()=>de,smokeRingFragmentShader:()=>Pe,smokeRingMeta:()=>ie,spiralFragmentShader:()=>qe,staticMeshGradientFragmentShader:()=>no,staticMeshGradientMeta:()=>ye,staticRadialGradientFragmentShader:()=>lo,staticRadialGradientMeta:()=>Ae,swirlFragmentShader:()=>Ze,swirlMeta:()=>xe,toProcessedGemSmoke:()=>ko,toProcessedHeatmap:()=>_o,toProcessedLiquidMetal:()=>Ao,voronoiFragmentShader:()=>He,voronoiMeta:()=>ge,warpFragmentShader:()=>Xe,warpMeta:()=>ve,waterFragmentShader:()=>uo,wavesFragmentShader:()=>Ke});var ke=`#version 300 es
...
}`;function Re(o){return"paperShaderMount"in o}function Go(){let o=navigator.userAgent.toLowerCase();return o.includes("safari")&&!o.includes("chrome")&&!o.includes("android")}function To(o){let e=visualViewport?.scale??1,a=visualViewport?.width??window.innerWidth,r=window.innerWidth-o.documentElement.clientWidth,u=e*a+r,n=outerWidth/u,c=Math.round(100*n);return c%5===0?c/100:c===33?1/3:c===67?2/3:c===133?4/3:n}var Me={fit:"contain",scale:1,rotation:0,offsetX:0,offsetY:0,originX:.5,originY:.5,worldWidth:0,worldHeight:0},Oe={fit:"none",scale:1,rotation:0,offsetX:0,offsetY:0,originX:.5,originY:.5,worldWidth:0,worldHeight:0},ze={none:0,contain:1,cover:2};var d=`
...
`;var ie={maxColorCount:10,maxNoiseIterations:8},Pe=`#version 300 es
precision mediump float;
...
vec2 fbm(vec2 n0, vec2 n1) {
  vec2 total = vec2(0.0);
  float amplitude = .4;
  for (int i = 0; i < ${ie.maxNoiseIterations}; i++) {
...
void main() {
  ...
  for (int i = ${ie.maxColorCount} - 2; i >= 0; i--) {
...
`;var pe={maxColorCount:10},Te=`#version 300 es
precision mediump float;
...
`,Qe={circle:0,diamond:1,square:2,triangle:3};var de={maxColorCount:10},We=`#version 300 es
precision mediump float;
...
`;var re={maxColorCount:8,maxBallsCount:20},Ye=`#version 300 es
precision mediump float;
...
void main() {
  ...
  for (int i = 0; i < ${re.maxBallsCount}; i++) {
    if (i >= int(ceil(u_count))) break;

    float idxFract = float(i) / float(${re.maxBallsCount});
...
`;var ge={maxColorCount:5},He=`#version 300 es
precision mediump float;
...
`;var ve={maxColorCount:10},Xe=`#version 300 es
precision mediump float;
...
`,je={checks:0,stripes:1,edge:2};var he={maxColorCount:5},Je=`#version 300 es
precision mediump float;
...
void main() {
  ...
  float outerShape = 0.;
  for (int i = 1; i < ${xe.maxColorCount+1}; i++) {
    if (i > int(u_colorsCount)) break;
...
`,eo={simplex:1,warp:2,dots:3,wave:4,ripple:5,swirl:6,sphere:7},oo={random:1,"2x2":2,"4x4":3,"8x8":4};var _e={maxColorCount:7},to=`#version 300 es
...
`,ao={wave:1,dots:2,truchet:3,corners:4,ripple:5,blob:6,sphere:7};var se={maxColorCount:5,maxSpots:4},io=`#version 300 es
precision lowp float;
...
void main() {
  ...
  for (int colorIdx = 0; colorIdx < ${se.maxColorCount}; colorIdx++) {
    ...
    for (int spotIdx = 0; spotIdx < ${se.maxSpots}; spotIdx++) {
  ...
  fragColor = vec4(color, opacity);
}`,ro={auto:0,square:1};var ne={maxColorCount:7},so=`#version 300 es
precision lowp float;
...
`;var ye={maxColorCount:10},no=`#version 300 es
precision mediump float;
...
`;var Ae={maxColorCount:10},lo=`#version 300 es
precision mediump float;
...
`,mo={lines:1,linesIrregular:2,wave:3,zigzag:4,pattern:5},po={prism:1,lens:2,contour:3,cascade:4,flat:5};var go=`#version 300 es
precision mediump float;
...
`;var Be={maxSamples:50},vo=`#version 300 es
precision mediump float;
...
`;var be={maxColorCount:10},xo=`#version 300 es
...
`,wo={classic:0,gooey:1,holes:2,soft:3},So={square:0,hex:1};var Bo=`#version 300 es
precision mediump float;
...
`,Uo={dots:0,ink:1,sharp:2};var Ce={maxColorCount:6},Vo=`#version 300 es
precision mediump float;
